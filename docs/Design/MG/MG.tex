...
\midrule
5/12/2019 & 1.0 & First version of document\\
...
\item[\refstepcounter{acnum} \actheacnum \label{reactants}:] The assumption that only two reactants are involved in the chemical reaction each consists of maximum two elements.
...
\item [\refstepcounter{mnum} \mthemnum \label{Reaction}:] Balancing Chemical Reaction Module 
...
\subsubsection{Balancing Chemical Reaction Module (\mref{Reaction})}
...
\midrule
R1 & \mref{mHH}, \mref{input} \\
R2 & \mref{mHH}, \mref{display}\\
R3 &   \mref{mHH},  \mref{Atomic Mass} ,\mref{Reaction} ,  \mref{Mass}\\
R4 & \mref{mHH}, \mref{input},  \mref{Atomic Mass} ,\mref{Reaction} , \mref{Mass}\\
\bottomrule
\end{tabular}
