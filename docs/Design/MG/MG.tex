...
\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] The structure and format of the input data with respect to chemical equations.
\item[\refstepcounter{acnum} \actheacnum \label{reactants}:] The assumption that only two reactants are involved in the chemical reaction.
\item[\refstepcounter{acnum} \actheacnum \label{compound}:] The assumption that maximum number of compound elements are two.
\item[\refstepcounter{acnum} \actheacnum \label{atomic mass}:] Underlying structure used to get molecular weight value 
\item[\refstepcounter{acnum} \actheacnum \label{calculation}:] The calculation method.
\item[\refstepcounter{acnum} \actheacnum \label{output}:] The format of the final output data.
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucIO}:] The goal statements of SMMP.
...
\item [\refstepcounter{mnum} \mthemnum \label{input}:] Input Module
\item [\refstepcounter{mnum} \mthemnum \label{Atomic Mass}:] Atomic Mass Module
\item [\refstepcounter{mnum} \mthemnum \label{Reaction}:] ReactionT Module 
\item [\refstepcounter{mnum} \mthemnum \label{Mass}:] Mass Calculation Module
\item [\refstepcounter{mnum} \mthemnum \label{display}:] GUI Module
\end{description}
...
\wss{I do not see where your other types will be defined.  There is a ReactionT,
  but what about the building block types to get there?}
...
\wss{Why isn't M1 used for anything?}
...
\wss{One module (M4) tied to two anticipated changes is not a good sign.  This
  should either be changed, or explicitly addressed in your write-up.}
...
\wss{I fixed one of the references.  Please fix the others.  The original file
  in the Blank Project Template should have worked with no modifications.}
\end{document}
